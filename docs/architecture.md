...
- **Tier 2 — Realization contract** (`skills/realization-contract/SKILL.md`)
  ...
  - the operations/interface a realization implements
  - the **input configuration schema** a realization requires from the
    consuming workspace (see §3) — published here, versioned, so both
    marketplace-shipped and workspace-authored realizations can declare
    conformance and be validated against it
...
- **Tier 3 — Realizations** (`skills/realize-<provider>/SKILL.md`, one
  per concrete provider, N+1 where N ≥ 0 — see §4)
  Each is a concrete implementation satisfying the Tier 2 contract for
  one specific provider/technology. Each realization's SKILL.md:
  - declares which concept + contract version it satisfies
  - publishes its own required input schema (a superset compatible with
    Tier 2's base schema — see §3)
...
- `config` is validated against **that realization's published input
  schema** (which must be compatible with — a superset of — the
  concept's Tier 2 base schema). Realizations are responsible for
  publishing this schema (§1, Tier 3) so tooling and the activation
...
3. Validates the `config` block against the active realization's
   published input schema (§3), reporting specific missing/invalid
   fields.
...
- Exact machine-readable format for the Tier 2/3 input schema (JSON
  Schema embedded in SKILL.md frontmatter vs. a sibling `schema.json`).
...
- Whether `guidance-activation-check` also offers a standalone
  "audit my whole settings.yml" skill, separate from the per-realization
